...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['toggle_status']) && isset($_POST['blog_id'])) {
        ...
        if ($row) {
            ...
            if ($newStatus === 'published' && $row['status'] === 'draft') {
                \App\Models\Subscriber::notifyNewPost(['id' => $blogId, 'title' => $row['title'] ?? '', 'content' => $row['content'] ?? '']);
            }
        }
        $qs = [];
    ...
    if (isset($_POST['blog_id'])) {
        ...
        if ($status === 'published' && $oldStatus !== 'published') {
            \App\Models\Subscriber::notifyNewPost(['id' => $id, 'title' => $title, 'content' => $content]);
        }
    } else {
        // Create new blog
